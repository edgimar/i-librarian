...
########## main body ##############
    ...
    if (isset ($_GET['searchname'])) $_SESSION['session_download_searchname'] = $_GET['searchname'];
    if (isset ($_GET['tagged_query'])) $_SESSION['session_download_tagged_query'] = $_GET['tagged_query'];
    if (isset ($_GET['pmid'])) $_SESSION['session_download_pmid'] = $_GET['pmid'];
    if (isset ($_GET['author'])) $_SESSION['session_download_author'] = $_GET['author'];
    if (isset ($_GET['title'])) $_SESSION['session_download_title'] = $_GET['title'];
    if (isset ($_GET['abstract'])) $_SESSION['session_download_abstract'] = $_GET['abstract'];
    if (isset ($_GET['journal'])) $_SESSION['session_download_journal'] = $_GET['journal'];
    if (isset ($_GET['year'])) $_SESSION['session_download_year'] = $_GET['year'];
    if (isset ($_GET['volume'])) $_SESSION['session_download_volume'] = $_GET['volume'];
    if (isset ($_GET['pagination'])) $_SESSION['session_download_pagination'] = $_GET['pagination'];
    if (isset ($_GET['edat'])) $_SESSION['session_download_edat'] = $_GET['edat'];
    if (isset ($_GET['sort'])) $_SESSION['session_download_sort'] = $_GET['sort'];
    if (isset ($_GET['limit'])) $_SESSION['session_download_limit'] = $_GET['limit'];
    if (isset ($_GET['last_search'])) $_SESSION['session_download_last_search'] = $_GET['last_search'];
    if (isset ($_GET['hasabstract'])) $_SESSION['session_download_hasabstract'] = $_GET['hasabstract'];
    if (isset ($_GET['review'])) $_SESSION['session_download_review'] = $_GET['review'];
    if (isset ($_GET['english'])) $_SESSION['session_download_english'] = $_GET['english'];
...
########## prepare PubMed query ##############
    ...
    if (!empty($_GET['author'])) $pubmed_query_array[] = "\"$_GET[author]\" [AU]";
    if (!empty($_GET['title'])) $pubmed_query_array[] = "$_GET[title] [TI]";
    if (!empty($_GET['abstract'])) $pubmed_query_array[] = "$_GET[abstract] [TIAB]";
    if (!empty($_GET['journal'])) $pubmed_query_array[] = "\"$_GET[journal]\" [TA]";
    if (!empty($_GET['year'])) $pubmed_query_array[] = "$_GET[year] [DP]";
    if (!empty($_GET['volume'])) $pubmed_query_array[] = "$_GET[volume] [VI]";
    if (!empty($_GET['pagination'])) $pubmed_query_array[] = "$_GET[pagination] [PG]";
    ...
    if (!empty($edat)) $pubmed_query = "$pubmed_query AND $edat [EDAT]";
    if (!empty($_GET['limit'])) $pubmed_query = "$pubmed_query AND $_GET[limit] [SB]";
    if (!empty($_GET['hasabstract'])) $pubmed_query = "$pubmed_query AND hasabstract";
    if (!empty($_GET['review'])) $pubmed_query = "$pubmed_query AND review [PT]";
    if (!empty($_GET['english'])) $pubmed_query = "$pubmed_query AND english [LA]";

    if (!empty($_GET['pmid'])) $pubmed_query = "$_GET[pmid] [PMID]";
...
########## display search result summaries ##############
    ...
    if (!empty($webenv) && !empty($querykey)) {
        ...
        if (!empty($_SESSION['session_download_searchname'])) print ': '.htmlspecialchars($_SESSION['session_download_searchname']);
        ...
        if ($count > 0) {
            ...
            print '<table cellspacing="0" class="top"><tr><td style="width: 20%">';
            ...
            print '<div class="ui-state-highlight ui-corner-top'.($retstart == 0 ? ' ui-state-disabled' : '').'" style="float:left;margin-left:2px;width:26px">'
                    .($retstart == 0 ? '' : '<a class="navigation" href="'.htmlspecialchars('download.php?webenv='.urlencode($webenv).'&querykey='.urlencode($querykey).'&retstart=0&count='.$count).'" style="display:block;width:26px">').
                    '<span class="ui-icon ui-icon-seek-first" style="margin-left:5px"></span>'
                    .($retstart == 0 ? '' : '</a>').
                    '</div>';

            print '<div class="ui-state-highlight ui-corner-top'.($retstart == 0 ? ' ui-state-disabled' : '').'" style="float:left;margin-left:2px">'
                    .($retstart == 0 ? '' : '<a class="navigation" href="'.htmlspecialchars('download.php?webenv='.urlencode($webenv).'&querykey='.urlencode($querykey).'&retstart='.($retstart-10).'&count='.$count).'" style="color:black;display:block;width:100%">').
            ...
            print '<div class="ui-state-highlight ui-corner-top'.($count > $retstart+10 ? '' : ' ui-state-disabled').'" style="float:right;margin-right:2px;width:26px">'
                    .($count > $retstart+10 ? '<a class="navigation" href="'.htmlspecialchars("download.php?webenv=".urlencode($webenv)."&querykey=".urlencode($querykey)."&retstart=$lastpage&count=$count").'" style="display:block;width:26px">' : '').
            ...
            print '<div class="ui-state-highlight ui-corner-top'.($count > $retstart+10 ? '' : ' ui-state-disabled').'" style="float:right;margin-right:2px">'
                    .($count > $retstart+10 ? '<a class="navigation" href="'.htmlspecialchars("download.php?webenv=".urlencode($webenv)."&querykey=".urlencode($querykey)."&retstart=".($retstart+10)."&count=$count").'" style="color:black;display:block;width:100%">' : '').
            ...
            foreach ($xml->DocSum as $docsum) {
                ...
                foreach ($docsum->Item as $c) {
                    ...
                    foreach ($c->attributes() as $a => $b) {
                        ...
                        if ($a == 'Name' && $b == 'Title') $title = $c[0];
                        if ($a == 'Name' && $b == 'Source') $journal = $c[0];
                        if ($a == 'Name' && $b == 'PubDate') $pub_date = $c[0];
                        if ($a == 'Name' && $b == 'DOI') $doi = $c[0];
                        if ($a == 'Name' && $b == 'ISSN') $issn = $c[0];
                        if ($a == 'Name' && $b == 'AuthorList') {
                            $author = array();
                            foreach ($c->Item as $d) {
                                foreach ($d->attributes() as $a => $b) {
                                    if ($a == 'Name' && $b == 'Author') $author[] = $d[0];
                                }
                            }
                ...
                if (isset($id) && isset($title) && isset($journal)) {
                    ...
                    if ($result) $relation = $result->fetchColumn();
                    $result = null;
                    ...
                    print '<div class="flag '.(($relation == 1) ? 'ui-state-error-text' : 'ui-priority-secondary').'" style="float:right;margin:4px"><span class="ui-icon ui-icon-flag"></span></div>';
                    
                    print '<div class="titles brief" style="overflow:hidden;margin-right:30px';

                    if (is_numeric($existing_id)) print ';color: #777';
                    ...
                    if (!empty($journal)) print htmlspecialchars($journal);
                    if (empty($journal) && !empty($secondary_title)) print htmlspecialchars($secondary_title);

                    if ($pub_date != '') print " ($pub_date)";
                    ...
                    if (!empty($pmcid)) print ' <b>&middot;</b> <a href="'.htmlspecialchars('http://www.ncbi.nlm.nih.gov/pmc/articles/'.urlencode($pmcid).'/pdf/').'" target="_blank">Full Text PDF</a> (PubMed Central)';
            ...
            print '<div class="ui-state-highlight ui-corner-bottom'.($retstart == 0 ? ' ui-state-disabled' : '').'" style="float:left;margin-left:2px">'
                    .($retstart == 0 ? '' : '<a class="navigation prevpage" href="'.htmlspecialchars('download.php?webenv='.urlencode($webenv).'&querykey='.urlencode($querykey).'&retstart='.($retstart-10).'&count='.$count).'" style="color:black;display:block;width:100%">').
                    '<span class="ui-icon ui-icon-triangle-1-w" style="float:left"></span>Back&nbsp;&nbsp;'
                    .($retstart == 0 ? '' : '</a>').
                    '</div>';
            ...
            print '<div class="ui-state-highlight ui-corner-bottom'.($count > $retstart+10 ? '' : ' ui-state-disabled').'" style="float:right;margin-right:2px">'
                    .($count > $retstart+10 ? '<a class="navigation nextpage" href="'.htmlspecialchars("download.php?webenv=".urlencode($webenv)."&querykey=".urlencode($querykey)."&retstart=".($retstart+10)."&count=$count").'" style="color:black;display:block;width:100%">' : '').
                    '<span class="ui-icon ui-icon-triangle-1-e" style="float:right"></span>&nbsp;&nbsp;Next'
                    .($count > $retstart+10 ? '</a>' : '').
                    '</div>';
...
<div style="text-align: left">
    <form enctype="application/x-www-form-urlencoded" action="download.php" method="GET" id="download-form">
        <input type="hidden" name="form_submitted" value="">
        <table cellspacing="0" class="threed" style="width:99%">
            <tr>
                <td style="border: 0;background-color: transparent;width:23em">
                ...
                <td style="border: 0px; background-color: transparent;text-align:right">
                    <a href="http://www.pubmed.org" TARGET="_blank">PubMed</a>
                </td>
            </tr>
            <tr>
                <td class="threed">
            ...
            <tr>
                ...
                <td class="threed">
                    <table cellspacing="0">
                        <tr><td class="select_span">
                                <input type="checkbox" name="hasabstract" style="display:none" <?php print !empty($_SESSION['session_download_hasabstract']) ? 'checked' : ''; ?>>
                                <span class="ui-icon ui-icon-<?php print !empty($_SESSION['session_download_hasabstract']) ? 'check' : 'close' ?>" style="float:left">
                                </span> have abstracts (hasabstract)</td></tr>
                        <tr><td class="select_span">
                                <input type="checkbox" name="review" style="display:none" <?php print !empty($_SESSION['session_download_review']) ? 'checked' : ''; ?>>
                                <span class="ui-icon ui-icon-<?php print !empty($_SESSION['session_download_review']) ? 'check' : 'close' ?>" style="float:left">
                                </span> are reviews (review [PT])</td></tr>
                        <tr><td class="select_span">
                                <input type="checkbox" name="english" style="display:none" <?php print !empty($_SESSION['session_download_english']) ? 'checked' : ''; ?>>
                                <span class="ui-icon ui-icon-<?php print !empty($_SESSION['session_download_english']) ? 'check' : 'close' ?>" style="float:left">
                                </span> are in English (english [LA])</td></tr>
                    </table>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="threed">
                    <input type="text" name="searchname" size="35" style="float:left;width:50%" value="<?php print isset($_SESSION['session_download_searchname']) ? htmlspecialchars($_SESSION['session_download_searchname']) : ''?>">
                    &nbsp;<button id="download-save">Save</button>
                </td>
            </tr>
        </table>
        &nbsp;<a href="http://www.ncbi.nlm.nih.gov/books/bv.fcgi?rid=helppubmed.chapter.pubmedhelp" TARGET="_blank">Help</a>
