...
while (list($cat_key,$cat_name) = each($cat_all)) {
	if(stristr("$paper[title] $paper[abstract]", $cat_name)) {
		if (in_array($cat_key, $cat_paper)) {
			$checkbox = '<div style="cursor: pointer"><span class="ui-icon ui-icon-check" style="float:left"></span>';
		} else {
			$checkbox = '<div style="cursor: pointer"><span class="ui-icon ui-icon-close" style="float:left"></span>';
		}
		$suggested_categories[] = $checkbox.$cat_name.'</div>';
	}
}
...
<input type="text" size="25" id="filter_categories" value="Filter" style="width:190px;margin-bottom:6px">
<div class="separator" style="margin:0"></div>
<input type="submit" value="Save" style="display:none">
<button id="newcatbutton" style="margin:6px 0">Save</button><br>
...
<td>
<table class="categorieslist" cellspacing=0 style="width: 49%;float: left;padding:2px">
<?php
	$i=1;
	$newdiv = null;
	...
	while (list($cat_key,$cat_name) = each($cat_all)){
		...
		if ($i > (1+$totalcount/2) && !$newdiv) {
			print '</table><table class="categorieslist" cellspacing=0 style="width: 49%;float: right;padding:2px">';
			$newdiv = true;
		}

		if (isset($cat_paper) && is_numeric(array_search($cat_key, $cat_paper))) $cat_selected = true;
		print PHP_EOL.'<tr><td class="select_span';
		if ($cat_selected) print  ' alternating_row" style="font-weight: bold';
		print '">';
		print '<input type="checkbox" name="category[]" value="'.htmlspecialchars($cat_key).'"';
		if ($cat_selected) print ' checked';
		print ' style="display:none">';
		print '<span class="ui-icon ui-icon-';
		print ($cat_selected) ? 'check': 'close';
		print '" style="float:left"></span>'.htmlspecialchars($cat_name).'</td></tr>';
