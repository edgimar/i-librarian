...
function list_dirs($directory) {
    ...
    while (list($key, $value) = each($dir_content)) {
        if (is_dir($directory . DIRECTORY_SEPARATOR . $value) && is_readable($directory . DIRECTORY_SEPARATOR . $value) && $value != '.' && $value != '..' && $value[0] != '.' && count($directories) < 2001) {
            $directories[] = $directory . DIRECTORY_SEPARATOR . $value;
            list_dirs($directory . DIRECTORY_SEPARATOR . $value);
        }
    }
    return $directories;
}
...
if (!isset($_GET['commence'])) {
    ...
    if (isset($_SESSION['auth']) && ($_SESSION['permissions'] == 'A' || $_SESSION['permissions'] == 'U')) {
        ...
        if (empty($_GET['directory'])) {
            ...
            $dbHandle = null;
            ?>
            <div style="margin:4px;font-weight:bold">PDF Batch Import</div><br><br>
            ...
            <div class="item-sticker ui-widget-content ui-corner-all" style="width:600px;margin:auto">
                <div class="alternating_row items ui-corner-top">
                    ...
                    <form id="batchimportform" action="batchimport.php" method="GET">
                        <input type="text" name="directory" value="<?php print $batchimport_dir; ?>" size="45" style="width:540px"><br>
                        <table cellspacing=0 style="margin: 2px 0px 2px 0px">
                            <tr>
                                <td class="select_span">
                                    ...
                                    <span class="ui-icon ui-icon-<?php print empty($batchimport_recursive) ? 'close' : 'check'  ?>" style="float:left"></span>
                ...
                <div class="alternating_row items ui-corner-bottom">
                    <button id="batchimportbutton">Import</button>
                    </form>
                </div>
            </div>
            <?php
            ...
            if (isset($_GET['recursive']))
                $directories = list_dirs($_GET['directory']);
            while (list($key, $directory) = each($directories)) {
                $glob = glob($directory . DIRECTORY_SEPARATOR . '*.pdf');
                $pdf_files = array_merge($pdf_files, $glob);
            }
            ...
            <table cellspacing=0 style="width:100%;margin-top: 4px;margin-bottom: 1px;">
                <tr>
                    <td>
                        <div class="ui-state-highlight ui-corner-top" style="float:right;margin-left:2px;margin-right:4px;padding-right:4px" id="open2">
                            <span class="ui-icon ui-icon-tag" style="float:left"></span>Categories
                        </div>
                        <div class="ui-state-highlight ui-corner-top clicked" style="float:right;padding-right:4px" id="open1">
                            <span class="ui-icon ui-icon-gear" style="float:left"></span>Options
                        </div>
                        <b>&nbsp;PDF Batch Import</b>
                    </td>
                </tr>
            </table>
                ...
                <table cellspacing="0" style="width: 100%;border-top: solid 1px #D5D6D9">
                    <tr>
                        <td valign="top" class="threedleft">
                            <button id="batchimportbutton2" <?php if ($pdf_count == 0)
                print 'disabled';
            ?>>Import</button>
                        </td>
                            ...
                            <table cellspacing="0">
                                <tr>
                                    <td class="select_span" style="line-height: 16px">
                                        ...
                                        <span class="ui-icon ui-icon-radio-<?php print (empty($batchimport_log) || (isset($batchimport_log) && $batchimport_log == '1')) ? 'on' : 'off'  ?>" style="float:left"></span>
                                ...
                                <tr>
                                    <td class="select_span" style="line-height: 16px">
                                        ...
                                        <span class="ui-icon ui-icon-radio-<?php print (isset($batchimport_log) && $batchimport_log == '2') ? 'on' : 'off'  ?>" style="float:left"></span>
                ...
                <table cellspacing="0" style="width:100%;display:none" id="table2">
                    <tr>
                        ...
                        <td class="threedright">
                            ...
                            <span class="ui-icon ui-icon-triangle-1-s enlargelist" style="cursor: pointer;float:left" id=""></span>
                            <span class="ui-icon ui-icon-triangle-1-n shrinklist" style="cursor: pointer;" id=""></span>
                        </td>
                    </tr>
                    <tr>
                        <td class="threedleft">
